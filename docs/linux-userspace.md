...
├── downloads/         the tarball, kept after a verified install: the repair rungs write base
│                      bytes from it, and a userspace that needs them is one whose connection
│                      may also be what is broken. Given up on uninstall, on a repair that
│                      could not otherwise fit on the device, and when the pin changes
├── tmp/               PROOT_TMP_DIR: proot's own scratch, one exec-<pid>-XXXXXX directory per
│                      proot process while it runs. A Start clears the ones whose process is gone
...
| Clear local state | The damage an interrupted package operation leaves on disk, and the one class of failure every other rung used to fail identically on: package-manager locks **that nothing holds** are removed, and the guest's own directories are put back — `/tmp` and `/run` recreated where they are missing or are a file or a link, the workspace the user deleted recreated, a workspace that is a symlink replaced by a real directory. proot's own scratch directories (`tmp/exec-<pid>-XXXXXX`, one per launch, which it removes at exit — see [Troubleshooting](#troubleshooting) for the exits where it could not) are cleared too, matched by the pid proot put in the name so a live session's is never touched and nothing under one is followed out of it. It only puts the rootfs back into a state the pipeline can work in; the pipeline itself runs once, in the rung below | Nothing the user owns: a stale lock is a file the kernel already released, nothing here is downloaded, and nothing apt has fetched is thrown away |
...
| Reinstall | The rootfs is thrown away and built from the pin again | The base system *and* the packages installed on top of it, written from the archive on disk |
...
reinstall, which costs the packages. `var/lib/dpkg/status` is the single deliberate exception, and it
is the reason that rung exists at all: the preserved-member rule made a corrupt database unfixable by
construction — no rung may write it, and every rung needs what is in it — so the rung that restores it
writes that one file and keeps the unreadable one beside it as `status.broken`.

Every rung that writes base bytes reads them from the archive the install already downloaded and
verified, which is kept on disk for exactly that reason: a userspace that needs a deep rung is one
whose connection may also be the thing that is broken, and a ladder that answered a broken userspace
by fetching tens of megabytes per rung would be answering it with the one resource that is not there.
It is given up in three places, each deliberate — uninstalling the userspace, the disk rung above
when a repair cannot otherwise fit on the device (with a warning that says so), and a pin the app no
longer matches, where the next fetch downloads over it. A repair therefore downloads nothing unless
the archive was never fetched, was given up for space, or no longer matches its pin; it clears apt's
lists only when the failure in front of it named them.
...
mirror that refuses or serves an unsigned index, a step that timed out, a disk with nothing left
to free, a clock that disagrees with the archive's own timestamps, a program killed by a signal (137
is the low-memory killer taking the largest process on the phone — no rung creates memory), the app's
own proot binary or loader missing from its installation, and every terminal the app can hold already
open. On a metered connection a rebuild is the one repair that leaves the user worse off, and it
...
| "the Linux runtime this app ships (libproot.so) is missing or unusable in this installation" | The app's own native library is not there or has no exec bit — an APK split installed for the wrong ABI, or a half-finished update. Nothing about Ubuntu is wrong | No Repair: no rung writes the app's installation, and every one of them would fail identically. Update or reinstall the app. The check runs before the fork, so the failure is this instead of a shell reporting 127 |
| `E: Could not get lock /var/lib/dpkg/lock-frontend`, `Unable to acquire the dpkg frontend lock`, after an install or a package command was killed | A lock *file* nothing holds: the kernel dropped the lock when the process died, but the file is still on disk and every `dpkg` and `apt-get` refuses in about a second because of it — with the same sentence, so every rung used to fail identically | Repair. Its local rung removes locks nothing holds (a lock something *does* hold is left alone) and downloads nothing. Verify names them: the probe's report lists the stale lock files |
| `dpkg: error: parsing file '/var/lib/dpkg/status' near line N`, or `dpkg` failing before it does anything at all after a kill mid-install | The package database is truncated — half a record, no final newline. `var/lib/dpkg` is a preserved member, so no rung used to be allowed to write it while every rung needed it | Repair: the local rung parks the unreadable file as `status.broken` and restores dpkg's own previous generation (`status-old`), which costs nothing and reinstalls nothing. With no `status-old` it writes the archive's copy — the base system's records only, so packages you installed read as not-installed while their files stay on disk; `apt install --reinstall` restores the record. If no source has one, Repair says so rather than emptying the file |
| A tool dying with "could not create temporary file", a shell that will not start, an apt that dies without a word — while the rest of the userspace passes | The guest's `/tmp` or `/run` is missing, is a *file*, or is a symlink out of the rootfs. Nothing else notices: a shell starts, `apt-get check` passes, and the fault surfaces only in some other program's words | Repair: the local rung recreates them as real directories (a link is replaced, never written through). Verify names them: `unusableTempDirs` |
| The terminal tab reports DISCONNECTED with status 127 and the card offered no Repair | The program the session was asked to run is not in the rootfs. The ending is about the *userspace*, not the session, but the tab's own view is "the shell ran and exited" | Repair: the ending now moves the userspace to NeedsRepair, but only when no other local session is live and only for 127 with no signal — a probe then decides, and a userspace the probe passes stays Running |
| The terminal dies with `Killed` (137) during a build or a package install, and the card offers Repair | The low-memory killer took the largest process on the phone. The userspace is intact | Close other apps and try again. Repair deliberately refuses this one: no rung creates memory, and the probe that would adjudicate forks another proot into the same shortage |
| `apt-get` fails with hash/404 errors | Stale archive pin or interrupted update | Repair. A setup run that fails over apt's own bookkeeping — a `Hash Sum mismatch`, a list it cannot parse, a half-written index — is the one case where the ladder gives up apt's lists, and the packages already downloaded with them, because apt reports a corrupt `.deb` in the same words. A failure that names neither leaves both trees alone and costs the user nothing. Check DNS in the probe report |
...
| `proot warning: cant chmod 'bash': Permission denied` (then one line per program), `proot error: cant remove '…/tmp/exec-<pid>-XXXXXX': Directory not empty`, and a health row saying the session runs as something other than `root` | One bug read from three sides, and nothing wrong with the userspace. proot reached its loader through a symlink named after the program being run (patch `0005`), and its teardown chmod'ed that symlink before unlinking it — `chmod` follows the link to the loader, at a path the platform allows the app to `execve` but not to `chmod`, so the chmod was refused, the unlink below it was skipped, and the directory was left holding its links: one per proot process. The warnings and `whoami`'s answer share one pty, so the health check read `root` plus proot's five lines as "the account" and called a working userspace not root | Fixed from both ends by the release that carries this page: proot patch `0006` unlinks a link instead of chmod'ing it, and the probe reads `whoami`'s answer line rather than the whole capture. What is already on disk needs neither — Repair's local rung clears the scratch directories of processes that are gone, and the health row corrects itself on the next probe. Packages install and `su` works the whole time this is showing |
...
| Install or Repair fails on space | The tarball plus the extracted rootfs plus apt's caches exceed what the volume has | Repair reclaims apt's caches, its lists, the temporary directories and the rotated logs before it does anything else, and reports how much it freed. If that is still too little for the repair to run, it gives up the kept archive as well and says so — the next rung that needs those bytes fetches them again. Uninstalling with "delete the workspace" is the other half: the rootfs is the large item |
